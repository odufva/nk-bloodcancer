...
# Plot top correlates in MM NK PRISM sensitivity in CCLE dataset (Figure 5B)
...
# load correlations
results <- fread("results_20Q4_complete/NK_PRISM_heme_correlations_MM.tsv", data.table = F)
...
fm_prism_clean <- fm_prism_clean[,subtype_short %in% "MM"]

# data frame with subtype for heatmap annotation
annot <- data.frame(cancertype = subtype_short[subtype_short %in% "MM"])
...
mat_gexp <- heatmap_scaled_matrix(DATAPAIR = "GEXP", ETRATIO = "AUC", GENES = c("CFLAR", "HLAIScore"))
mat_gnab <- heatmap_scaled_matrix(DATAPAIR = "GNAB", ETRATIO = "AUC", SCALE = F, GENES = c("TRAF3"))
...
ha1 <- HeatmapAnnotation(df = annot, col = list(`PRISM AUC` = colorRamp2(seq(-2, 2, length.out = 20), viridis_pal(direction = -1)(20)),
                                                Subtype = structure(c(colorRampPalette(brewer.pal(11, "PuOr")[2:10])(6)[1:3], "grey70"), 
                                                                    names = c("WHSC1", "CCND1", "MAF", "NA"))
),
gap = unit(0.1, "mm"),
...
`PRISM AUC` = list(title = "PRISM AUC\nZ-score", title_gp = gpar(fontsize = 5), 
                   title_position = "topcenter",
                   labels_gp = gpar(fontsize = 4), grid_height = unit(0.1, "cm"), grid_width = unit(2, "mm"),
                   legend_direction = "horizontal", at = c(-2,2),
                   labels = c("Low", "High"),
                   border = NA
)),
...
pdf("CCLE_NK_PRISM_heatmap_multiomic_MM_manuscript_selected.pdf", height = 2.95, width = 1.15)
