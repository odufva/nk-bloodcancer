...
## Basic QC
dir.create("/Users/sgandalf85/Dropbox (HRUH)/NK_resistance Heme CollabPaper/Analysis/CROP_seq_MM1S_activ/R/results", showWarnings = T)
dir.create("/Users/sgandalf85/Dropbox (HRUH)/NK_resistance Heme CollabPaper/Analysis/CROP_seq_MM1S_activ/R/results/mm1sa/", showWarnings = T)
dir.create("/Users/sgandalf85/Dropbox (HRUH)/NK_resistance Heme CollabPaper/Analysis/CROP_seq_MM1S_activ/R/results/mm1sa/qc/", showWarnings = F)
dir.create("/Users/sgandalf85/Dropbox (HRUH)/NK_resistance Heme CollabPaper/Analysis/CROP_seq_MM1S_activ/R/results/mm1sa/qc/before_qc1/", showWarnings = F)
dir.create("/Users/sgandalf85/Dropbox (HRUH)/NK_resistance Heme CollabPaper/Analysis/CROP_seq_MM1S_activ/R/results/mm1sa/qc/after_qc1/", showWarnings = F)
...
crop_seurat %>% plotQC(folder = "results/mm1sa/qc/before_qc1/")
...
ggsave("results/mm1sa/qc/after_qc1/scatter_clustering.png", width = 5, height = 4)
...
ggsave("results/mm1sa/qc/after_qc1/umap.png", width = 5, height = 4)
...
ggsave("results/mm1sa/qc/after_qc1/umap_ident.png", width = 5, height = 4)
...
ggsave("results/mm1sa/qc/after_qc1/umap_singler_hpca.png", width = 7, height = 4)
...
ggsave("results/mm1sa/qc/after_qc1/umap_blue_blueprint.png", width = 7, height = 4)
...
ggsave("results/mm1sa/qc/after_qc1/umap_dbl.png", width = 5, height = 4)
...
fwrite(all_deg, "results/mm1sa/qc/after_qc1/all_deg.txt", sep = "\t", quote = F, row.names = F)

saveRDS(crop_seurat, "results/mm1sa/qc/after_qc1/mm1sa_cropseq_seurat.rds")
getwd()
setwd("/Users/sgandalf85/Dropbox (HRUH)/NK_resistance Heme CollabPaper/Analysis/CROP_seq_MM1S_activ/R/")
######## Add the sgRNA information
dir.create("results/mm1sa/qc/sgrna_qc/", showWarnings = F)

cropseq_no_nk_data <- fread("data/sg/mm1s_activ_cropseq_cr.txt") %>% dplyr::rename(sequence = barcode, barcode = cell) %>% mutate(barcode = paste0("MM1S_CROPseq_activ", barcode))
cropseq_nk_1_16_data    <- fread("data/sg/nk2587_1_16_mm1s_activ_cropseq_cr.txt") %>% dplyr::rename(sequence = barcode, barcode = cell) %>% mutate(barcode = paste0("NK2587_1_16_MM1S_CROPseq_activ", barcode))
cropseq_nk_1_4_data    <- fread("data/sg/nk2587_1_4_mm1s_activ_cropseq_cr.txt") %>% dplyr::rename(sequence = barcode, barcode = cell) %>% mutate(barcode = paste0("NK2587_1_4_MM1S_CROPseq_activ", barcode))
cropseq_library    <- fread("data/sg/MM1S_CRISPRa_CROP_seq_sgRNA_library.txt")
...
fwrite(cropseq_data, "results/mm1sa/qc/sgrna_qc/cropseq_data.txt", sep = "\t", quote = F, row.names = F)
...
ggsave("results/mm1sa/qc/sgrna_qc/scatter_top_fraction_counts.png", width = 5, height = 4)

ggplot(plotdata, aes(prop)) + geom_histogram(bins = 100) +
  labs(x = "top sgRNA \nUMI fraction", y = "count")
ggsave("results/mm1sa/qc/sgrna_qc/histogram_top_fraction.png", width = 5, height = 4)
...
ggsave("results/mm1sa/qc/sgrna_qc/scatter_top1_fraction_top2_counts.png", width = 5, height = 4)
...
rank_df %>%
  ...
  add_guide +
  scale_color_manual(values = c("lightgrey", "salmon", "dodgerblue"))
ggsave("results/mm1sa/qc/sgrna_qc/scatter_top1_fraction_top1_counts.png", width = 5, height = 4)
...
rank_df %>%
  ...
  add_guide +
  scale_color_manual(values = c("lightgrey", "salmon", "dodgerblue"))
ggsave("results/mm1sa/qc/sgrna_qc/scatter_top2_fraction_top2_counts.png", width = 5, height = 4)
...
rank_df %>%
  ...
  add_guide +
  scale_color_manual(values = c("lightgrey", "salmon", "dodgerblue"))
ggsave("results/mm1sa/qc/sgrna_qc/scatter_top2_fraction_top1_counts.png", width = 5, height = 4)

fwrite(cropseq_data_dup, "results/mm1sa/qc/sgrna_qc/cropseq_data_duplicated_barcodes.txt", sep = "\t", quote = F, row.names = F)
fwrite(rank_df, "results/mm1sa/qc/sgrna_qc/rank_df.txt", sep = "\t", quote = F, row.names = F)
fwrite(label_df, "results/mm1sa/qc/sgrna_qc/label_df.txt", sep = "\t", quote = F, row.names = F)
...
fwrite(cropseq_labels, "results/mm1sa/qc/sgrna_qc/cropseq_labels_in_seurat.txt", sep = "\t", quote = F, row.names = F)


## Add to seurat
#crop_seurat <- readRDS("results/mm1sa/qc/after_qc1/mm1sa_cropseq_seurat.rds")
...
saveRDS(crop_seurat, "results/mm1sa/qc/after_qc1/mm1sa_crop_seurat_qc1.rds")
...
ggsave("results/mm1sa/qc/after_qc1/umap_pert.png", width = 10, height = 8)

## ------------------------------------------------------------

## QC2: Remove clusters 5-n (NK cells/low quality?)

dir.create("results/mm1sa/qc/after_qc2/", showWarnings = F)
...
## Decide on clustering
crop_seurat %>% plotClustering()
ggsave("results/mm1sa/qc/after_qc2/scatter_clustering.png", width = 5, height = 4)
...
ggsave("results/mm1sa/qc/after_qc2/umap.png", width = 5, height = 4)

DimPlot(crop_seurat, group.by = "orig.ident", label = T, repel = T) + theme_bw(base_size = 12) + scale_color_manual(values = getPalette3(6)[c(1,4,5)]) + labs("UMAP 1", "UMAP 2") + theme(legend.position = "top")
ggsave("results/mm1sa/qc/after_qc2/umap_ident.png", width = 5, height = 4)

DimPlot(crop_seurat, group.by = "singler_hpca_pred", label = T, repel = T) + theme_bw(base_size = 12) + scale_color_manual(values = getPalette2(10)) + labs("UMAP 1", "UMAP 2") + theme(legend.position = "right")
ggsave("results/mm1sa/qc/after_qc2/umap_singler_hpca.png", width = 7, height = 4)

DimPlot(crop_seurat, group.by = "singler_blueprint_pred", label = T, repel = T) + theme_bw(base_size = 12) + scale_color_manual(values = getPalette2(11)) + labs("UMAP 1", "UMAP 2") + theme(legend.position = "right")
ggsave("results/mm1sa/qc/after_qc2/umap_blue_blueprint.png", width = 7, height = 4)

FeaturePlot(crop_seurat, features = "hybrid_doublet_score", label = T, repel = T, cols = c("lightgrey", "tomato"), min.cutoff = 0) + theme_bw(base_size = 12)  + labs("UMAP 1", "UMAP 2") + theme(legend.position = "right")
ggsave("results/mm1sa/qc/after_qc2/umap_dbl.png", width = 5, height = 4)

crop_seurat %>% plotQC(folder = "results/qc/after_qc2/")
dev.off()

all_deg <- FindAllMarkers(crop_seurat, test.use = "t", only.pos = T, return.thresh = 0.05)
fwrite(all_deg, "results/mm1sa/qc/after_qc2/all_deg.txt", sep = "\t", quote = F, row.names = F)

saveRDS(crop_seurat, "results/mm1sa/qc/after_qc2/mm1sa_cropseq_seurat.rds")

## QC3: Remove small NK cell cluster 
dir.create("results/mm1sa/qc/after_qc3/", showWarnings = F)
crop_seurat <- subset(crop_seurat, idents = c(0:6,8))
crop_seurat <- crop_seurat %>% preprocessSeurat(cells.to.use = colnames(crop_seurat)) %>% getClustering()

## Decide on clustering
crop_seurat %>% plotClustering()
ggsave("results/mm1sa/qc/after_qc3/scatter_clustering.png", width = 5, height = 4)
...
ggsave("results/mm1sa/qc/after_qc3/sgrna_doublet_nCount_RNA.png", width = 5, height = 4)

VlnPlot(crop_seurat, features = "bcds_doublet_score_norm", split.by = "status")
ggsave("results/mm1sa/qc/after_qc3/sgrna_doublet_bcds_score.png", width = 5, height = 4)
...
ggsave("results/mm1sa/qc/after_qc3/cell_doublet.png", width = 5, height = 4)

VlnPlot(subset(crop_seurat, cell_doublet == "singlet"), features = "nCount_RNA", split.by = "status")
ggsave("results/mm1sa/qc/after_qc2/sgrna_doublet_nCount_RNA_filtered.png", width = 5, height = 4)
...
saveRDS(crop_seurat, "results/mm1sa/mm1sa_crop_seurat_withdoublets_2.rds")
...
ggsave("results/mm1sa/qc/after_qc3/umap.png", width = 5, height = 4)
...
ggsave("results/mm1sa/qc/after_qc3/umap_ident.png", width = 7, height = 4)
...
ggsave("results/mm1sa/qc/after_qc3/umap_singler_hpca.png", width = 7, height = 4)
...
ggsave("results/mm1sa/qc/after_qc3/umap_blue_blueprint.png", width = 7, height = 4)
...
ggsave("results/mm1sa/qc/after_qc3/umap_phase.png", width = 5.5, height = 4)

FeaturePlot(crop_seurat, features = "hybrid_doublet_score", label = T, repel = T, cols = c("lightgrey", "tomato"), min.cutoff = 0) + theme_bw(base_size = 12)  + labs("UMAP 1", "UMAP 2") + theme(legend.position = "right")
ggsave("results/mm1sa/qc/after_qc3/umap_dbl.png", width = 5, height = 4)

all_deg <- FindAllMarkers(crop_seurat, test.use = "t", only.pos = T, return.thresh = 0.05, logfc.threshold = 0.05)
fwrite(all_deg, "results/mm1sa/qc/after_qc3/all_deg.txt", sep = "\t", quote = F, row.names = F)
all_deg %>% group_by(cluster) %>% top_n(20, dplyr::desc(avg_log2FC)) %>% View

saveRDS(crop_seurat, "results/mm1sa/mm1sa_crop_seurat_singlet_2.rds")
...
DimPlot(crop_seurat, split.by = "gene", ncol = 5, label = T, repel = T) +
  ...
  labs("UMAP 1", "UMAP 2") +
  theme(legend.position = "none")
ggsave("results/mm1sa/qc/after_qc3/umap_pert.png", width = 10, height = 8)
