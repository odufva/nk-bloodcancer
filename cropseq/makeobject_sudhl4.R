...
files <- list.files(recursive = T, include.dirs = T) %>% 
  grep(pattern = "CROPseq_SUDHL4", value = T) %>% 
...
## Basic QC
dir.create("results/sudhl4/", showWarnings = F)
dir.create("results/sudhl4/qc/", showWarnings = F)
dir.create("results/sudhl4/qc/before_qc1/", showWarnings = F)
dir.create("results/sudhl4/qc/after_qc1/", showWarnings = F)
...
crop_seurat %>% plotQC(folder = "results/sudhl4/qc/before_qc1/")
...
ggsave("results/sudhl4/qc/after_qc1/scatter_clustering.png", width = 5, height = 4)
...
ggsave("results/sudhl4/qc/after_qc1/umap.png", width = 5, height = 4)
...
ggsave("results/sudhl4/qc/after_qc1/umap_ident.png", width = 5, height = 4)
...
ggsave("results/sudhl4/qc/after_qc1/umap_singler_hpca.png", width = 7, height = 4)
...
ggsave("results/sudhl4/qc/after_qc1/umap_blue_blueprint.png", width = 7, height = 4)
...
ggsave("results/sudhl4/qc/after_qc1/umap_dbl.png", width = 5, height = 4)
...
fwrite(all_deg, "results/sudhl4/qc/after_qc1/all_deg.txt", sep = "\t", quote = F, row.names = F)

saveRDS(crop_seurat, "results/sudhl4/qc/after_qc1/sudhl4_cropseq_seurat.rds")

######## Add the sgRNA information
dir.create("results/sudhl4/qc/sgrna_qc/", showWarnings = F)

cropseq_no_nk_data <- fread("data/CROPseq_SUDHL4_noNK_sgRNA.txt") %>% dplyr::rename(sequence = barcode, barcode = cell) %>% mutate(barcode = paste0("CROPseq_SUDHL4_noNK_", barcode))
cropseq_nk_1_16_data    <- fread("data/CROPseq_SUDHL4_NK1_1_16_sgRNA.txt") %>% dplyr::rename(sequence = barcode, barcode = cell) %>% mutate(barcode = paste0("CROPseq_SUDHL4_NK1_1_16_", barcode))
cropseq_nk_1_4_data    <- fread("data/CROPseq_SUDHL4_NK1_1_4_sgRNA.txt") %>% dplyr::rename(sequence = barcode, barcode = cell) %>% mutate(barcode = paste0("CROPseq_SUDHL4_NK1_1_4_", barcode))
cropseq_library    <- fread("data/CROPseq_SUDHL4_sgRNA_library.txt")
...
fwrite(cropseq_data, "results/sudhl4/qc/sgrna_qc/cropseq_data.txt", sep = "\t", quote = F, row.names = F)
...
ggsave("results/sudhl4/qc/sgrna_qc/scatter_top_fraction_counts.png", width = 5, height = 4)

ggplot(plotdata, aes(prop)) + geom_histogram(bins = 100) +
  labs(x = "top sgRNA \nUMI fraction", y = "count")
ggsave("results/sudhl4/qc/sgrna_qc/histogram_top_fraction.png", width = 5, height = 4)
...
ggsave("results/sudhl4/qc/sgrna_qc/scatter_top1_fraction_top2_counts.png", width = 5, height = 4)
...
rank_df %>%
  ...
  add_guide +
  scale_color_manual(values = c("lightgrey", "salmon", "dodgerblue"))
ggsave("results/sudhl4/qc/sgrna_qc/scatter_top1_fraction_top1_counts.png", width = 5, height = 4)
...
rank_df %>%
  ...
  add_guide +
  scale_color_manual(values = c("lightgrey", "salmon", "dodgerblue"))
ggsave("results/sudhl4/qc/sgrna_qc/scatter_top2_fraction_top2_counts.png", width = 5, height = 4)
...
rank_df %>%
  ...
  add_guide +
  scale_color_manual(values = c("lightgrey", "salmon", "dodgerblue"))
ggsave("results/sudhl4/qc/sgrna_qc/scatter_top2_fraction_top1_counts.png", width = 5, height = 4)

fwrite(cropseq_data_dup, "results/sudhl4/qc/sgrna_qc/cropseq_data_duplicated_barcodes.txt", sep = "\t", quote = F, row.names = F)
fwrite(rank_df, "results/sudhl4/qc/sgrna_qc/rank_df.txt", sep = "\t", quote = F, row.names = F)
fwrite(label_df, "results/sudhl4/qc/sgrna_qc/label_df.txt", sep = "\t", quote = F, row.names = F)
...
fwrite(cropseq_labels, "results/sudhl4/qc/sgrna_qc/cropseq_labels_in_seurat.txt", sep = "\t", quote = F, row.names = F)


## Add to seurat
#crop_seurat <- readRDS("results/sudhl4/qc/after_qc1/sudhl4_cropseq_seurat.rds")
...
saveRDS(crop_seurat, "results/sudhl4/qc/after_qc1/sudhl4_crop_seurat_qc1.rds")
...
ggsave("results/sudhl4/qc/after_qc1/umap_pert.png", width = 10, height = 8)
...
# load object
#crop_seurat <- readRDS("results/sudhl4/qc/after_qc1/sudhl4_cropseq_seurat.rds")
#crop_seurat <- readRDS("results/sudhl4/sudhl4_crop_seurat.rds")

dir.create("results/sudhl4/qc/after_qc2/", showWarnings = F)
crop_seurat <- subset(crop_seurat, idents = 0:4)
crop_seurat <- crop_seurat %>% preprocessSeurat(cells.to.use = colnames(crop_seurat)) %>% getClustering()

## Decide on clustering
crop_seurat %>% plotClustering()
ggsave("results/sudhl4/qc/after_qc2/scatter_clustering.png", width = 5, height = 4)
...
ggsave("results/sudhl4/qc/after_qc2/sgrna_doublet_nCount_RNA.png", width = 5, height = 4)

VlnPlot(crop_seurat, features = "bcds_doublet_score_norm", split.by = "status")
ggsave("results/sudhl4/qc/after_qc2/sgrna_doublet_bcds_score.png", width = 5, height = 4)
...
ggsave("results/sudhl4/qc/after_qc2/cell_doublet.png", width = 5, height = 4)

VlnPlot(subset(crop_seurat, cell_doublet == "singlet"), features = "nCount_RNA", split.by = "status")
ggsave("results/sudhl4/qc/after_qc2/sgrna_doublet_nCount_RNA_filtered.png", width = 5, height = 4)
...
saveRDS(crop_seurat, "results/sudhl4/sudhl4_crop_seurat_withdoublets.rds")
...
DimPlot(crop_seurat, group.by = "RNA_snn_res.0.3", label = T, repel = T) + theme_bw(base_size = 12) + scale_color_manual(values = getPalette3(8)) + labs("UMAP 1", "UMAP 2") + theme(legend.position = "none")
ggsave("results/sudhl4/qc/after_qc2/umap.png", width = 5, height = 4)
...
ggsave("results/sudhl4/qc/after_qc2/umap_ident.png", width = 7, height = 4)
...
ggsave("results/sudhl4/qc/after_qc2/umap_singler_hpca.png", width = 7, height = 4)
...
ggsave("results/sudhl4/qc/after_qc2/umap_blue_blueprint.png", width = 7, height = 4)
...
ggsave("results/sudhl4/qc/after_qc2/umap_phase.png", width = 5.5, height = 4)

FeaturePlot(crop_seurat, features = "hybrid_doublet_score", label = T, repel = T, cols = c("lightgrey", "tomato"), min.cutoff = 0) + theme_bw(base_size = 12)  + labs("UMAP 1", "UMAP 2") + theme(legend.position = "right")
ggsave("results/sudhl4/qc/after_qc2/umap_dbl.png", width = 5, height = 4)

all_deg <- FindAllMarkers(crop_seurat, test.use = "t", only.pos = T, return.thresh = 0.05, logfc.threshold = 0.05)
fwrite(all_deg, "results/sudhl4/qc/after_qc2/all_deg.txt", sep = "\t", quote = F, row.names = F)
all_deg %>% group_by(cluster) %>% top_n(20, dplyr::desc(avg_log2FC)) %>% View

saveRDS(crop_seurat, "results/sudhl4/sudhl4_crop_seurat_singlet.rds")
...
DimPlot(crop_seurat, split.by = "gene", ncol = 5, label = T, repel = T) +
  ...
  labs("UMAP 1", "UMAP 2") +
  theme(legend.position = "none")
ggsave("results/sudhl4/qc/after_qc2/umap_pert.png", width = 10, height = 8)
