...
# NK cell co-culture with panel of 26 cell lines, hashing scRNA-seq analysis (donor NK1)
...
files <- list.files(recursive = T, include.dirs = T) %>% 
  grep(pattern = "Batch1|Batch2", value = T) %>% 
  grep(pattern = "HTO", value = T, invert = T)

# get files for each sample/lane
files1 <- files %>% grep(pattern = "Batch1_NK1", value = T) %>% grep(pattern = "PBMC", value = T, invert = T)
files2 <- files %>% grep(pattern = "Batch1_NK1_PBMC", value = T)
files3 <- files %>% grep(pattern = "Batch1_targets", value = T)
files4 <- files %>% grep(pattern = "Batch2_NK1", value = T) %>% grep(pattern = "PBMC", value = T, invert = T)
files5 <- files %>% grep(pattern = "Batch2_NK1_PBMC", value = T)
files6 <- files %>% grep(pattern = "Batch2_targets", value = T)
...
hash_seurat    <- merge(scrnaseq_files1, y = c(scrnaseq_files2, scrnaseq_files3, scrnaseq_files4, scrnaseq_files5, scrnaseq_files6), add.cell.ids = extractBatchName(c(files1, files2, files3, files4, files5, files6)))


## Basic QC
dir.create("./results/celllinepanel_nk1/qc/", recursive = T, showWarnings = F)
dir.create("./results/celllinepanel_nk1/qc/before_qc1/", recursive = T, showWarnings = F)
dir.create("./results/celllinepanel_nk1/qc/after_qc1/", recursive = T, showWarnings = F)
dir.create("./results/celllinepanel_nk1/qc/hto/", recursive = T, showWarnings = F)
...
hash_seurat %>% plotQC(folder = "results/celllinepanel_nk1/qc/before_qc1/", min_mito = 0, max_mito = 15, min_ribo = 5, max_ribo = 50, min_features = 300, max_features = 10e3,
...
ggsave("results/celllinepanel_nk1/qc/before_qc1/scatter_clustering.png", width = 5, height = 4)


Idents(hash_seurat_beforeQC) <- hash_seurat_beforeQC$RNA_snn_res.0.8
hash_seurat_beforeQC$cluster <- Idents(hash_seurat_beforeQC)

## Visualize
getPalette3 <- colorRampPalette(brewer.pal(9, "Set3"))

DimPlot(hash_seurat_beforeQC, group.by = "RNA_snn_res.0.8", label = T, repel = T) + theme_bw(base_size = 12) + scale_color_manual(values = getPalette3(60)) + labs("UMAP 1", "UMAP 2") + theme(legend.position = "none")
ggsave("results/celllinepanel_nk1/qc/before_qc1/umap.png", width = 8, height = 6)

# Plot cell type marker genes
FeaturePlot(hash_seurat_beforeQC, features = c("GZMA", "CD79A", "CD3E", "JCHAIN", "MPO", "HLA-DRA", "Cnk"), label = T, repel = T, cols = c("lightgrey", "tomato"), min.cutoff = 0) & theme_bw(base_size = 12)  & labs("UMAP 1", "UMAP 2") & theme(legend.position = "right")
ggsave("results/celllinepanel_nk1/qc/before_qc1/umap_genes.png", width = 20, height = 15)

# Plot experiments
DimPlot(hash_seurat_beforeQC, group.by = "RNA_snn_res.0.8", split.by = "orig.ident", label = T, repel = T, ncol = 3) + theme_bw(base_size = 12) + scale_color_manual(values = getPalette3(60)) + labs("UMAP 1", "UMAP 2") + theme(legend.position = "none")
ggsave("results/celllinepanel_nk1/qc/before_qc1/umap_experiments.png", width = 24, height = 12)

#all_deg <- FindAllMarkers(hash_seurat_beforeQC, test.use = "t", only.pos = T, return.thresh = 0.05)
#fwrite(all_deg, "results/celllinepanel_nk1/qc/before_qc1/all_deg.txt", sep = "\t", quote = F, row.names = F)
...
ggsave("results/celllinepanel_nk1/qc/after_qc1/scatter_clustering.png", width = 5, height = 4)
...
ggsave("results/celllinepanel_nk1/qc/after_qc1/umap.png", width = 5, height = 4)
...
ggsave("results/celllinepanel_nk1/qc/after_qc1/umap_experiments.png", width = 24, height = 12)
...
ggsave("results/celllinepanel_nk1/qc/after_qc1/umap_cycle.png", width = 5, height = 4)
...
ggsave("results/celllinepanel_nk1/qc/after_qc1/umap_singler_hpca.png", width = 14, height = 7)

DimPlot(hash_seurat, group.by = "singler_blueprint_pred", label = T, repel = T) + theme_bw(base_size = 12) + scale_color_manual(values = getPalette3(21)) + labs("UMAP 1", "UMAP 2") + theme(legend.position = "right")
ggsave("results/celllinepanel_nk1/qc/after_qc1/umap_singler_blueprint.png", width = 10, height = 6)
...
ggsave("results/celllinepanel_nk1/qc/after_qc1/umap_dbl.png", width = 5, height = 4)

hash_seurat %>% plotQC(folder = "results/celllinepanel_nk1/qc/after_qc1/")

#all_deg <- FindAllMarkers(hash_seurat, test.use = "t", only.pos = T, return.thresh = 0.05)
#fwrite(all_deg, "results/celllinepanel_nk1/qc/after_qc1/all_deg.txt", sep = "\t", quote = F, row.names = F)
...
# get files for each sample/lane
files1_hto <- files_hto %>% grep(pattern = "Batch1_NK1", value = T) %>% grep(pattern = "PBMC", value = T, invert = T)
files2_hto <- files_hto %>% grep(pattern = "Batch1_NK1_PBMC", value = T)
files3_hto <- files_hto %>% grep(pattern = "Batch1_targets", value = T)
files4_hto <- files_hto %>% grep(pattern = "Batch2_NK1", value = T) %>% grep(pattern = "PBMC", value = T, invert = T)
files5_hto <- files_hto %>% grep(pattern = "Batch2_NK1_PBMC", value = T)
files6_hto <- files_hto %>% grep(pattern = "Batch2_targets", value = T)

# Create Seurat objects of each sample/lane and get sample/lane names from filenames
extractBatchName <- function(filenames){gsub("data/|_matrix.mtx.gz", "", filenames[grepl("mtx", filenames)])}

## Load HTO count matrices
htos_1_nk_pbmc <- ReadMtx(files1_hto[grepl("mtx", files1_hto)], files1_hto[grepl("barcodes", files1_hto)], files1_hto[grepl("features", files1_hto)], feature.column = 1)
htos_1_nk_pbmc <- htos_1_nk_pbmc[1:14,-1]

htos_1_nk <- ReadMtx(files2_hto[grepl("mtx", files2_hto)], files2_hto[grepl("barcodes", files2_hto)], files2_hto[grepl("features", files2_hto)], feature.column = 1)
htos_1_nk <- htos_1_nk[1:14,-1]

htos_1_targets <- ReadMtx(files3_hto[grepl("mtx", files3_hto)], files3_hto[grepl("barcodes", files3_hto)], files3_hto[grepl("features", files3_hto)], feature.column = 1)
htos_1_targets <- htos_1_targets[1:13,-1]

htos_2_nk_pbmc <- ReadMtx(files4_hto[grepl("mtx", files4_hto)], files4_hto[grepl("barcodes", files4_hto)], files4_hto[grepl("features", files4_hto)], feature.column = 1)
htos_2_nk_pbmc <- htos_2_nk_pbmc[1:14,-1]

htos_2_nk <- ReadMtx(files5_hto[grepl("mtx", files5_hto)], files5_hto[grepl("barcodes", files5_hto)], files5_hto[grepl("features", files5_hto)], feature.column = 1)
htos_2_nk <- htos_2_nk[1:14,-1]

htos_2_targets <- ReadMtx(files6_hto[grepl("mtx", files6_hto)], files6_hto[grepl("barcodes", files6_hto)], files6_hto[grepl("features", files6_hto)], feature.column = 1)
htos_2_targets <- htos_2_targets[1:13,-1]

## Load HTO ids
hto_id <- as.data.frame(
          rbind(fread("data/Batch1_NK1_PBMC_HTO_sample_list.txt"),
                fread("data/Batch1_NK1_HTO_sample_list.txt"),
                fread("data/Batch1_targets_HTO_sample_list.txt"),
                fread("data/Batch2_NK1_PBMC_HTO_sample_list.txt"),
                fread("data/Batch2_NK1_HTO_sample_list.txt"),
                fread("data/Batch2_targets_HTO_sample_list.txt")))
...
runDemultiplex <- function(hto, prefix, experiment){
  ...
  rownames(htos) <- hto_id_subset$sample[match(gsub("^.*\\-", "", rownames(htos)), hto_id_subset$hto_sequence)]
  ...
  dir.create(paste0("results/celllinepanel_nk1/qc/hto/", experiment))
  ...
  ggsave(paste0("results/celllinepanel_nk1/qc/hto/", experiment, "/ridgeplot.pdf"), height = 10, width = 30)
  ...
  VlnPlot(hash_seurat_demux, features = "nCount_RNA", pt.size = 0.1, log = TRUE) +
    ...
    xlab("") &
    theme(plot.title = element_text(face = "plain", hjust = 0.5))
  ggsave(paste0("results/celllinepanel_nk1/qc/hto/", experiment, "/ncount_rna_violinplot.pdf"), height = 4, width = 4)
  ...
  dir.create(paste0("results/celllinepanel_nk1/hto_singlets/", experiment), recursive = T, showWarnings = F)
  ...
  ggsave(paste0("results/celllinepanel_nk1/hto_singlets/", experiment, "/tsne.pdf"), height = 5, width = 6)
  ...
  ggsave(paste0("results/celllinepanel_nk1/hto_singlets/", experiment, "/heatmap.pdf"), height = 4, width = 8)
  
  return(hash_seurat_demux)
  
}

hash_seurat_demux_1_nk_pbmc <- runDemultiplex(htos_1_nk_pbmc, "Batch1_NK1_PBMC_", "batch1_nk_pbmc")
hash_seurat_demux_1_nk <- runDemultiplex(htos_1_nk, "Batch1_NK1_", "batch1_nk")
hash_seurat_demux_1_targets<- runDemultiplex(htos_1_targets, "Batch1_targets_", "batch1_targets")
hash_seurat_demux_2_nk_pbmc <- runDemultiplex(htos_2_nk_pbmc, "Batch2_NK1_PBMC_", "batch2_nk_pbmc")
hash_seurat_demux_2_nk <- runDemultiplex(htos_2_nk, "Batch2_NK1_", "batch2_nk")
hash_seurat_demux_2_targets<- runDemultiplex(htos_2_targets, "Batch2_targets_", "batch2_targets")

# extract sample identities and merge with original object
labels <- data.frame(barcode = c(names(hash_seurat_demux_1_nk_pbmc$hash.ID), names(hash_seurat_demux_1_nk$hash.ID), names(hash_seurat_demux_1_targets$hash.ID),
                                 names(hash_seurat_demux_2_nk_pbmc$hash.ID), names(hash_seurat_demux_2_nk$hash.ID), names(hash_seurat_demux_2_targets$hash.ID)),
                     hash.ID =  c(as.character(hash_seurat_demux_1_nk_pbmc$hash.ID), as.character(hash_seurat_demux_1_nk$hash.ID), as.character(hash_seurat_demux_1_targets$hash.ID),
                                  as.character(hash_seurat_demux_2_nk_pbmc$hash.ID), as.character(hash_seurat_demux_2_nk$hash.ID), as.character(hash_seurat_demux_2_targets$hash.ID)))
...
hash_seurat_singlet <- FindClusters(hash_seurat_singlet, resolution = 0.8, verbose = FALSE)
...
ggsave("results/celllinepanel_nk1/hto_singlets/umap.png", width = 7, height = 5)
...
ggsave("results/celllinepanel_nk1/hto_singlets/umap_cycle.png", width = 6, height = 4)

# Save object
saveRDS(hash_seurat_singlet, "results/celllinepanel_nk1/hash_seurat_singlet.rds")
...
DimPlot(hash_seurat_singlet, split.by = "hash.ID", ncol = 9, group.by = "seurat_clusters") +
  ...
  scale_color_manual(values = rev(getPalette(41))) +
  labs("UMAP 1", "UMAP 2")
ggsave("results/celllinepanel_nk1/hto_singlets/hto_class_umap.pdf", height = 16, width = 16)
...
DimPlot(hash_seurat_singlet, split.by = "hash.ID", ncol = 9, group.by = "seurat_clusters") +
  ...
  scale_color_manual(values = rev(getPalette(41))) +
  labs("UMAP 1", "UMAP 2")
ggsave("results/celllinepanel_nk1/hto_singlets/hto_class_umap_large.pdf", height = 32, width = 32)
...
DimPlot(hash_seurat_singlet, split.by = "hash.ID", ncol = 9, group.by = "singler_blueprint_pred") +
  ...
  scale_color_manual(values = rev(getPalette(41))) +
  labs("UMAP 1", "UMAP 2")
ggsave("results/celllinepanel_nk1/hto_singlets/hto_class_umap_large_singler.pdf", height = 32, width = 32)
...
DimPlot(hash_seurat_singlet, group.by = "hash.ID", label = T, repel = T) +
  ...
  theme(axis.ticks = element_blank(),
        ...
        plot.title = element_blank(),
        strip.background = element_blank()) +
  scale_color_manual(values = rev(getPalette(80))) +
  labs("UMAP 1", "UMAP 2") +
  NoLegend()
ggsave("results/celllinepanel_nk1/hto_singlets/hto_class_umap_colored.pdf", height = 10, width = 16)
...
DimPlot(hash_seurat_singlet, split.by = "orig.ident", ncol = 3, group.by = "seurat_clusters", label = T, repel = T) +
  ...
  scale_color_manual(values = rev(getPalette(41))) +
  labs("UMAP 1", "UMAP 2")
ggsave("results/celllinepanel_nk1/hto_singlets/umap_experiment.png", height = 16, width = 24)
...
DimPlot(hash_seurat_singlet, split.by = "orig.ident", ncol = 3, group.by = "hash.ID", label = T, repel = T) +
  ...
  labs("UMAP 1", "UMAP 2") +
  NoLegend()
ggsave("results/celllinepanel_nk1/hto_singlets/umap_experiment_samples.png", height = 12, width = 16)
...
DimPlot(hash_seurat_singlet, group.by = "singler_blueprint_pred", label = T, repel = T) +
  ...
  theme(axis.ticks = element_blank(),
        ...
        plot.title = element_blank(),
        strip.background = element_blank()) +
  scale_color_manual(values = rev(getPalette(22))) +
  labs("UMAP 1", "UMAP 2")
ggsave("results/celllinepanel_nk1/hto_singlets/singler_blueprint_pred.pdf", height = 7, width = 12)
...
DimPlot(hash_seurat_singlet, group.by = "singler_hpca_pred", label = T, repel = T) +
  ...
  scale_color_manual(values = rev(getPalette(41))) +
  labs("UMAP 1", "UMAP 2")
ggsave("results/celllinepanel_nk1/hto_singlets/singler_hpca_pred.pdf", height = 7, width = 12)

## ---------------------------------------

# Save processed object
hash_seurat_singlet <- readRDS("results/celllinepanel_nk1/celllinepanel_nk1_seurat.rds")
...
ggplot(cell_numbers, aes(x = Var1, y = Freq, fill = Var1)) +
  geom_col() +
  scale_fill_manual(values = getPalette3(80)) +
...
ggsave("results/celllinepanel_nk1/hto_singlets/hto_cell_numbers.pdf", height = 5, width = 20)
...
cell_numbers <- data.frame(sample = hash_seurat_singlet$hash.ID, cluster = hash_seurat_singlet$seurat_clusters) %>% 
  mutate(celltype = ifelse(cluster %in% c(1, 5, 28), "NK cell", "Target cell")) %>% 
  select(-cluster)

cell_numbers <- data.frame(table(cell_numbers)) %>%
  mutate(treatment = ifelse(grepl("expanded", sample), "NK-expanded",
                            ifelse(grepl("PBMC", sample), "NK-PBMC", "Targets only"))) %>% 
...
ggsave("results/celllinepanel_nk1/hto_singlets/hto_cell_numbers_celltype.pdf", height = 7, width = 10)
