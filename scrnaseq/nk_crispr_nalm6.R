...
# NK cell co-culture with edited NALM6 hashing scRNA-seq analysis
...
## Basic QC
dir.create("./results/nalm6/qc/", recursive = T, showWarnings = F)
dir.create("./results/nalm6/qc/before_qc1/", recursive = T, showWarnings = F)
dir.create("./results/nalm6/qc/after_qc1/", recursive = T, showWarnings = F)
dir.create("./results/nalm6/qc/hto/", recursive = T, showWarnings = F)
...
hash_seurat %>% plotQC(folder = "results/nalm6/qc/before_qc1/")
...
ggsave("results/nalm6/qc/before_qc1/scatter_clustering.png", width = 5, height = 4)
...
DimPlot(hash_seurat_beforeQC, group.by = "RNA_snn_res.0.7", label = T, repel = T) + theme_bw(base_size = 12) + scale_color_manual(values = getPalette3(15)) + labs("UMAP 1", "UMAP 2") + theme(legend.position = "none")
ggsave("results/nalm6/qc/before_qc1/umap.png", width = 5, height = 4)

# Plot B cell genes (NALM6?)
FeaturePlot(hash_seurat_beforeQC, features = c("CD79A", "CD19"), label = T, repel = T, cols = c("lightgrey", "tomato"), min.cutoff = 0) & theme_bw(base_size = 12)  & labs("UMAP 1", "UMAP 2") & theme(legend.position = "right")
ggsave("results/nalm6/qc/before_qc1/umap_CD79A_CD19.png", width = 10, height = 4)

all_deg <- FindAllMarkers(hash_seurat_beforeQC, test.use = "t", only.pos = T, return.thresh = 0.05)
fwrite(all_deg, "results/nalm6/qc/before_qc1/all_deg.txt", sep = "\t", quote = F, row.names = F)
...
Idents(hash_seurat) <- hash_seurat$RNA_snn_res.0.7
hash_seurat$cluster <- Idents(hash_seurat)

## Visualize
getPalette3 <- colorRampPalette(brewer.pal(9, "Set3"))

DimPlot(hash_seurat, group.by = "RNA_snn_res.0.7", label = T, repel = T) + theme_bw(base_size = 12) + scale_color_manual(values = getPalette3(11)) + labs("UMAP 1", "UMAP 2") + theme(legend.position = "none")
...
ggsave("results/nalm6/qc/after_qc1/umap_cycle.png", width = 5, height = 4)
...
hash_seurat %>% plotQC(folder = "results/nalm6/qc/after_qc1/")
...
# get files for each sample/lane
files1_hto <- files_hto %>% grep(pattern = "NALM6", value = T) 
...
## Load HTO ids
hto_id <- fread("data/NK1_NALM6_CRISPR_HTO_sample_list.txt", data.table = F)
...
# Save object
saveRDS(hash_seurat, "results/nalm6/hash_seurat.rds")

# Demultiplex cells based on HTO enrichment
hash_seurat_demux <- HTODemux(hash_seurat, assay = "HTO", positive.quantile = 0.99)
...
ggsave("results/nalm6/qc/hto/ridgeplot.pdf", height = 10, width = 30)
...
p1 <- FeatureScatter(subset(hash_seurat_demux, HTO_classification.global == "Singlet"),
                     feature1 = "hto_sgCtrl.1", feature2 = "hto_sgFADD.4",
                     cols = getPalette3(14))

p2 <- FeatureScatter(subset(hash_seurat_demux, HTO_classification.global == "Singlet"),
                     feature1 = "hto_sgCtrl.1", feature2 = "hto_NK-only",
                     cols = getPalette3(14))

p1 + p2
ggsave("results/nalm6/qc/hto/scatterplots.pdf", height = 5, width = 12)
...
VlnPlot(hash_seurat_demux, features = "nCount_RNA", pt.size = 0.1, log = TRUE) +
  ...
  xlab("") &
  theme(plot.title = element_text(face = "plain", hjust = 0.5))
ggsave("results/nalm6/qc/hto/ncount_rna_violinplot.pdf", height = 4, width = 4)
...
dir.create("results/nalm6/hto_singlets/", recursive = T, showWarnings = F)
...
ggsave("results/nalm6/hto_singlets/tsne.pdf", height = 5, width = 6)
...
ggsave("results/nalm6/hto_singlets/heatmap.pdf", height = 4, width = 8)
...
hash_seurat_singlet$HTO_classification_grouped <- factor(hash_seurat_singlet$HTO_classification_grouped,
                                                         levels = c("NK", "Ctrl", "FADD", "SPPL3", "CMIP", "SPN"))
...
ggsave("results/nalm6/hto_singlets/umap.png", width = 7, height = 4)
...
ggsave("results/nalm6/hto_singlets/umap_cycle.png", width = 6, height = 4)

# Save object
saveRDS(hash_seurat_singlet, "results/nalm6/hash_seurat_singlet.rds")
