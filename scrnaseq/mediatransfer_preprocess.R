...
files <- list.files(recursive = T, include.dirs = T) %>% 
  grep(pattern = "Media_transfer", value = T) %>% 
...
## Basic QC
dir.create("./results/media_transfer/qc/", recursive = T, showWarnings = F)
dir.create("./results/media_transfer/qc/before_qc1/", recursive = T, showWarnings = F)
dir.create("./results/media_transfer/qc/after_qc1/", recursive = T, showWarnings = F)
dir.create("./results/media_transfer/qc/hto/", recursive = T, showWarnings = F)
...
hash_seurat %>% plotQC(folder = "results/media_transfer/qc/before_qc1/", min_mito = 0, max_mito = 15, min_ribo = 5, max_ribo = 50, min_features = 300, max_features = 10e3,
...
ggsave("results/media_transfer/qc/before_qc1/scatter_clustering.png", width = 5, height = 4)


Idents(hash_seurat_beforeQC) <- hash_seurat_beforeQC$RNA_snn_res.0.7
hash_seurat_beforeQC$cluster <- Idents(hash_seurat_beforeQC)

## Visualize
getPalette3 <- colorRampPalette(brewer.pal(9, "Set3"))

DimPlot(hash_seurat_beforeQC, group.by = "RNA_snn_res.0.7", label = T, repel = T) + theme_bw(base_size = 12) + scale_color_manual(values = getPalette3(26)) + labs("UMAP 1", "UMAP 2") + theme(legend.position = "none")
ggsave("results/media_transfer/qc/before_qc1/umap.png", width = 8, height = 6)
...
ggsave("results/media_transfer/qc/before_qc1/umap_genes.png", width = 20, height = 15)

# Plot experiments
DimPlot(hash_seurat_beforeQC, group.by = "RNA_snn_res.0.7", split.by = "orig.ident", label = T, repel = T, ncol = 3) + theme_bw(base_size = 12) + scale_color_manual(values = getPalette3(77)) + labs("UMAP 1", "UMAP 2") + theme(legend.position = "none")
ggsave("results/media_transfer/qc/before_qc1/umap_experiments.png", width = 24, height = 12)

#all_deg <- FindAllMarkers(hash_seurat_beforeQC, test.use = "t", only.pos = T, return.thresh = 0.05)
#fwrite(all_deg, "results/media_transfer/qc/before_qc1/all_deg.txt", sep = "\t", quote = F, row.names = F)
...
ggsave("results/media_transfer/qc/after_qc1/scatter_clustering.png", width = 5, height = 4)
...
ggsave("results/media_transfer/qc/after_qc1/umap.png", width = 5, height = 4)
...
ggsave("results/media_transfer/qc/after_qc1/umap_experiments.png", width = 24, height = 12)
...
ggsave("results/media_transfer/qc/after_qc1/umap_cycle.png", width = 5, height = 4)
...
ggsave("results/media_transfer/qc/after_qc1/umap_singler_hpca.png", width = 14, height = 7)
...
ggsave("results/media_transfer/qc/after_qc1/umap_singler_blueprint.png", width = 10, height = 6)
...
ggsave("results/media_transfer/qc/after_qc1/umap_dbl.png", width = 5, height = 4)

hash_seurat %>% plotQC(folder = "results/media_transfer/qc/after_qc1/")

#all_deg <- FindAllMarkers(hash_seurat, test.use = "t", only.pos = T, return.thresh = 0.05)
#fwrite(all_deg, "results/media_transfer/qc/after_qc1/all_deg.txt", sep = "\t", quote = F, row.names = F)
...
hto_id <- as.data.frame(
  rbind(fread("data/Media_transfer_plate1_HTO_sample_list.txt"),
        fread("data/Media_transfer_plate2_HTO_sample_list.txt")))
...
runDemultiplex <- function(hto, prefix, experiment){
  ...
  dir.create(paste0("results/media_transfer/qc/hto/", experiment))
  ...
  ggsave(paste0("results/media_transfer/qc/hto/", experiment, "/ridgeplot.pdf"), height = 10, width = 30)
  ...
  VlnPlot(hash_seurat_demux, features = "nCount_RNA", pt.size = 0.1, log = TRUE) +
    ...
    xlab("") &
    theme(plot.title = element_text(face = "plain", hjust = 0.5))
  ggsave(paste0("results/media_transfer/qc/hto/", experiment, "/ncount_rna_violinplot.pdf"), height = 4, width = 4)
  ...
  dir.create(paste0("results/media_transfer/hto_singlets/", experiment), recursive = T, showWarnings = F)
  ...
  ggsave(paste0("results/media_transfer/hto_singlets/", experiment, "/tsne.pdf"), height = 5, width = 6)
  ...
  ggsave(paste0("results/media_transfer/hto_singlets/", experiment, "/heatmap.pdf"), height = 4, width = 8)
  
  return(hash_seurat_demux)
  
}

hash_seurat_demux_plate1 <- runDemultiplex(htos_plate1, "Media_transfer_plate1_", "media_transfer_plate1")
hash_seurat_demux_plate2 <- runDemultiplex(htos_plate2, "Media_transfer_plate2_", "media_transfer_plate2")
...
DimPlot(hash_seurat_singlet, group.by = "seurat_clusters", label = T, repel = T) +
  ...
  scale_color_manual(values = rev(getPalette(16))) +
  labs("UMAP 1", "UMAP 2") +
...
ggsave("results/media_transfer/hto_singlets/umap.png", width = 7, height = 5)
...
ggsave("results/media_transfer/hto_singlets/umap_cycle.png", width = 6, height = 4)

# Save object
saveRDS(hash_seurat_singlet, "results/media_transfer/hash_seurat_singlet.rds")
...
DimPlot(hash_seurat_singlet, split.by = "hash.ID", ncol = 9, group.by = "seurat_clusters") +
  ...
  scale_color_manual(values = rev(getPalette(16))) +
  labs("UMAP 1", "UMAP 2")
ggsave("results/media_transfer/hto_singlets/hto_class_umap.pdf", height = 16, width = 16)
...
DimPlot(hash_seurat_singlet, split.by = "hash.ID", ncol = 9, group.by = "seurat_clusters") +
  ...
  scale_color_manual(values = rev(getPalette(16))) +
  labs("UMAP 1", "UMAP 2")
ggsave("results/media_transfer/hto_singlets/hto_class_umap_large.pdf", height = 32, width = 32)
...
DimPlot(hash_seurat_singlet, split.by = "hash.ID", ncol = 9, group.by = "singler_blueprint_pred") +
  ...
  theme(axis.ticks = element_blank(),
        ...
        plot.title = element_blank(),
        strip.background = element_blank()) +
  scale_color_manual(values = rev(getPalette(10))) +
  labs("UMAP 1", "UMAP 2")
ggsave("results/media_transfer/hto_singlets/hto_class_umap_large_singler.pdf", height = 32, width = 32)
...
DimPlot(hash_seurat_singlet, group.by = "hash.ID", label = T, repel = T) +
  ...
  theme(axis.ticks = element_blank(),
        ...
        plot.title = element_blank(),
        strip.background = element_blank()) +
  scale_color_manual(values = rev(getPalette(26))) +
  labs("UMAP 1", "UMAP 2") +
  NoLegend()
ggsave("results/media_transfer/hto_singlets/hto_class_umap_colored.pdf", height = 10, width = 16)
...
DimPlot(hash_seurat_singlet, split.by = "orig.ident", ncol = 3, group.by = "seurat_clusters", label = T, repel = T) +
  ...
  labs("UMAP 1", "UMAP 2")
ggsave("results/media_transfer/hto_singlets/umap_experiment.png", height = 16, width = 24)
...
DimPlot(hash_seurat_singlet, split.by = "orig.ident", ncol = 3, group.by = "hash.ID", label = T, repel = T) +
  ...
  labs("UMAP 1", "UMAP 2") +
  NoLegend()
ggsave("results/media_transfer/hto_singlets/umap_experiment_samples.png", height = 12, width = 16)
...
DimPlot(hash_seurat_singlet, group.by = "singler_blueprint_pred", label = T, repel = T) +
  ...
  scale_color_manual(values = rev(getPalette(10))) +
  labs("UMAP 1", "UMAP 2")
ggsave("results/media_transfer/hto_singlets/singler_blueprint_pred.pdf", height = 7, width = 12)
...
DimPlot(hash_seurat_singlet, group.by = "singler_hpca_pred", label = T, repel = T) +
  ...
  scale_color_manual(values = rev(getPalette(10))) +
  labs("UMAP 1", "UMAP 2")
ggsave("results/media_transfer/hto_singlets/singler_hpca_pred.pdf", height = 7, width = 12)
...
ggplot(cell_numbers, aes(x = Var1, y = Freq, fill = Var1)) +
  geom_col() +
  scale_fill_manual(values = getPalette3(26)) +
...
ggsave("results/media_transfer/hto_singlets/hto_cell_numbers.pdf", height = 5, width = 25)
...
ggsave("results/media_transfer/hto_singlets/hto_cell_numbers_celltype.pdf", height = 7, width = 10)
